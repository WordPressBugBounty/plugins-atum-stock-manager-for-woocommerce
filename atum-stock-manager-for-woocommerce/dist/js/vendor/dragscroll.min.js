	...
	var reset = function(i, el) {
		...
		for (i = 0; i < dragged.length;) {
			(function(el, lastClientX, lastClientY, startX, startY, moved, pushed, scroller, cont) {
				mousedown.split(" ").forEach(function(ev, index) {
					(cont = el.container || el)[addEventListener](ev, cont["md" + index] = function(e) {
						if (!e.target.classList.contains("no-drag") && (!el.hasAttribute("nochilddrag") || _document.elementFromPoint(e.pageX, e.pageY) == cont)) {
							fixTouches(e);
							pushed = 1;
							moved = 0;
				...
				(cont = el.container || el)[addEventListener](click, cont.mc = function(e) {
					if (moved) {
						e.preventDefault();
						e.stopPropagation();
						moved = 0;
				...
				mouseup.split(" ").forEach(function(ev, index) {
					_window[addEventListener](ev, cont["mu" + index] = function(e) {
						setTimeout(function() {
				...
				_document[addEventListener](mouseenter, cont.me = function(e) {
					if (!e.buttonsPressed) pushed = 0;
				}, 0);
				mousemove.split(" ").forEach(function(ev, index) {
					_window[addEventListener](ev, cont["mm" + index] = function(e) {
						if (pushed) {
							fixTouches(e);
							el.classList.add("dragging");
							if (!moved && (Math.abs(e.clientX - startX) > moveThreshold || Math.abs(e.clientY - startY) > moveThreshold)) {
								moved = true;
							}
							if (moved) {
